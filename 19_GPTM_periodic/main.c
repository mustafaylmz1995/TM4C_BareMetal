...
void GPTM_Timer0_periodic(unsigned int us);
...
int main(){
	...
	while(1){
		...
		GPION->DATA ^= (LED2);
		GPTM_Timer0_periodic(500*US2MS);
		GPION->DATA ^= (LED1);
		GPTM_Timer0_periodic(500*US2MS);
	}
...
void GPTM_Timer0_periodic(unsigned int us){ // Send us type, for instance 1 sec is equal to 1*10^9 
